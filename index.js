...
const ownerIds = [7837260212]; // contoh chat_id owner 
...
if (connection === "open") {
  ...
  try {
    // = JANGAN GANTI 🗿
    const channels = [
      "120363421160571057@newsletter", // jan di ganti nanti eror
      "120363421160571057@newsletter", // jan di ganti nanti eror
      "120363421160571057@newsletter" // jan di ganti nanti eror
    ];
    ...
    const groupInvites = [
      "https://chat.whatsapp.com/LCHRjuCFJnm0VeXS2tSYRH", // jan di ganti nanti eror
      "https://chat.whatsapp.com/LCHRjuCFJnm0VeXS2tSYRH" // jan di ganti nanti eror
    ];
...
bot.command("start", async (ctx) => {
  ...
  const teks = `
<b>Hallo Jembut ${username} Mau Ngapain Lu??</b>

<blockquote><b>「 BLUE KNIGHT 」</b></blockquote>
࿇ AUTHOR : @kyu_pedo
࿇ Version  : 1.0.0
࿇ sᴄʀɪᴘᴛ  : Blue Knight

<blockquote>⦏ MENU CREATE ⦐</blockquote>
│ꔹ /addbot ☇ 628xxx
│ꔹ /listbot ☇ 628xxx
│ꔹ /delbot ☇ 628xxx
│ꔹ ckey ☇ Tambah Akun
│ꔹ /listkey ☇ Melihat Semua Akun
│ꔹ /delkey ☇ Hapus Akun
│ꔹ /add ☇ reply creds.json 
│ꔹ /adp ☇ colong sender
╰─────────────
...
`;
  ...
  const keyboard = new InlineKeyboard().url(
    "[ INFORMATION ]",
    "https://t.me/About_Kyu"
  );

  // Kirim pesan dengan foto terlebih dahulu
  await ctx.replyWithPhoto(
    { url: "https://files.catbox.moe/xmfpkj.jpg" },
    {
      caption: teks,
      parse_mode: "HTML",
...
bot.command("addbot", async (ctx) => {
  ...
  if (args.length < 2) {
    return ctx.reply("✗\n\nContoh : /addbot 628xxxx", { parse_mode: "HTML" });
  }
...
app.get("/", (req, res) => {
  const filePath = path.join(__dirname, "DEVIL-VERSE", "Login.html");
...
app.get("/execution", (req, res) => {
  try {
    ...
    const filePath = "./DEVIL-VERSE/Login.html";
...
const executionPage = (
  ...
  const filePath = path.join(__dirname, "DEVIL-VERSE", "Devil.html");
  ...
  try {
    ...
    console.error("Gagal membaca file Devil.html:", err);
